...
// TODO: remove these
struct S_uninit { };
constexpr Reference(S_uninit&&)
 : m_index(index_uninit) { }
Reference& operator=(S_uninit&&)
 { return this->set_uninit();  }

struct S_void { };
constexpr Reference(S_void&&)
 : m_index(index_void) { }
Reference& operator=(S_void&&)
 { return this->set_void();  }

struct S_temporary { Value val; };
using S_constant = S_temporary;
Reference(S_temporary&& r)
 : m_value(::std::move(r.val)), m_index(index_temporary) { }
Reference& operator=(S_temporary&& r)
 { return this->set_temporary(::std::move(r.val));  }

struct S_variable { rcfwdp<Variable> var; };
Reference(S_variable&& r)
 : m_var(::std::move(r.var)), m_index(index_variable) { }
Reference& operator=(S_variable&& r)
 { return this->set_variable(::std::move(r.var));  }

struct S_ptc_args { rcfwdp<PTC_Arguments> ptca; };
Reference(S_ptc_args&& r)
 : m_ptca(::std::move(r.ptca)), m_index(index_ptc_args) { }
Reference& operator=(S_ptc_args&& r)
 { return this->set_ptc_args(::std::move(r.ptca));  }
